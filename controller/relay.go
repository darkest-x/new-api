...
func processChannelError(c *gin.Context, channelError types.ChannelError, err *types.NewAPIError) {
	...
	if constant.ErrorLogEnabled && types.IsRecordErrorLog(err) {
		...
		if logErr := model.RecordErrorLog(c, userId, channelId, modelName, tokenName, err.MaskSensitiveErrorWithStatusCode(), tokenId, useTimeSeconds, common.GetContextKeyBool(c, constant.ContextKeyIsStream), userGroup, other); logErr != nil {
			// P4-5: 错误审计日志写入失败不阻断错误响应，但必须 SysError 记录完整上下文供排查。
			// Other 字段含 error_type / error_code / status_code / channel_name / admin_info 等关键错误审计信息，
			//   若日志未落库这些信息全部丢失，运维需通过本 SysError 定位需人工排查的请求。
			// 注意：logErr 为 DB 写入错误（error 类型），err 仍指外层 *types.NewAPIError，
			//   status_code 从 err.StatusCode 取，DB 错误用 logErr 输出。
			common.SysError(fmt.Sprintf(
				"RecordErrorLog failed: log_persist_required=true, user_id=%d, channel_id=%d, model_name=%s, status_code=%d, log_type=error, err=%v",
				userId, channelId, modelName, err.StatusCode, logErr,
			))
		}
	}
...
func RelayMidjourney(c *gin.Context) {
	...
	logger.LogError(c, fmt.Sprintf("midjourney error: %v", mjErr))
	if mjErr != nil {
