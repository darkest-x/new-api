...
func FixChannelsAbilities(c *gin.Context) {
	success, fails, cacheRefreshErr, err := model.FixAbility()
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	if cacheRefreshErr != nil {
		// P4-5: abilities 表已成功重建，但内存缓存刷新失败。
		// 不返回 HTTP error（DB 写入已完成），仅在响应中标记 cache_refresh_required 供管理员感知。
		adminUserId := c.GetInt("id")
		common.SysError(fmt.Sprintf(
			"FixChannelsAbilities: cache refresh failed after ability rebuild: cache_refresh_required=true, channel_success=%d, channel_fails=%d, admin_user_id=%d, err=%v",
			success, fails, adminUserId, cacheRefreshErr,
		))
		data["cache_refresh_required"] = true
		data["cache_refresh_error"] = cacheRefreshErr.Error()
	}
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		...
		for _, tag := range tags {
			if tag != nil && *tag != "" {
				...
				err := sortOptions.Apply(buildChannelListQuery(group, -1, -1).Where("tag = ?", *tag)).
					Omit("key").
					Limit(model.MaxChannelSearchResults).
					Find(&tagChannels).Error
				if err != nil {
					c.JSON(http.StatusOK, gin.H{
						"success": false,
...
// P4-5 修复：返回 (bool, error)，DB 失败时 fail-closed 返回 error，
// 避免原本启用 2FA 的账户在 DB 故障期间被当作验证通过。
func validateTwoFactorAuth(twoFA *model.TwoFA, code string) (bool, error) {
	// 尝试验证TOTP
	if cleanCode, err := common.ValidateNumericCode(code); err == nil {
		isValid, err := twoFA.ValidateTOTPAndUpdateUsage(cleanCode)
		if err != nil {
			return false, err
		}
		if isValid {
	...
	// 尝试验证备用码
	isValid, err := twoFA.ValidateBackupCodeAndUpdateUsage(code)
	if err != nil {
		return false, err
	}
	if isValid {
...
func FetchModels(c *gin.Context) {
	...
	// P1-2 修复：使用 NewRequestWithContext
