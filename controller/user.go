...
func Login(c *gin.Context) {
	if !common.GetPasswordLoginEnabled() {
	...
	// 检查是否启用2FA
	// P4-5 修复：IsTwoFAEnabled 返回 (bool, error)，DB error 时 fail-closed 拒绝登录，
	// 避免原本启用 2FA 的账户在 DB 故障期间被当作未启用绕过两步验证。
	twoFAEnabled, err := model.IsTwoFAEnabled(user.Id)
	if err != nil {
		common.SysError(fmt.Sprintf("Login: IsTwoFAEnabled DB error for user %s (id=%d): %v", username, user.Id, err))
		common.ApiErrorI18n(c, i18n.MsgDatabaseError)
		return
	}
	if twoFAEnabled {
...
func Register(c *gin.Context) {
	...
	if !common.GetPasswordRegisterEnabled() {
	...
	if common.GetEmailVerificationEnabled() {
...
func GenerateAccessToken(c *gin.Context) {
	...
	// 检查 access_token 是否已存在。必须先检查 Error，再用 RowsAffected 判断：
	// DB 错误时 RowsAffected=0，若直接用 RowsAffected 判断会误认为"未重复"而放行。
	result := model.DB.Where("access_token = ?", user.AccessToken).First(user)
...
func ManageUser(c *gin.Context) {
	...
	case "delete":
		...
		// P4-5：缓存失效失败仅 SysError，不阻断用户删除流程（DB 已落库）。
		//   但若未及时刷新，被删用户的令牌可能在 TTL 内仍可访问，运维需通过本告警核对 Redis 状态。
		if err := model.InvalidateUserTokensCache(user.Id); err != nil {
			common.SysError(fmt.Sprintf(
				"InvalidateTokensCache failed: user_id=%d action=%s role=%d cache_invalidate_required=true err=%v",
				user.Id, req.Action, user.Role, err,
			))
		}
	case "promote":
	...
	case "add_quota":
		...
		case "override":
			...
			recordManageAuditFor(c, user.Id, "user.quota_override", map[string]interface{}{
				"from": logger.FormatQuota(oldQuota),
		...
		return
	default:
		// 未知 action 必须失败，不允许 fall-through 到 user.Update。
		common.ApiError(c, errors.New("未知操作"))
		return
	}
	...
	if req.Action == "disable" || req.Action == "promote" || req.Action == "demote" {
		...
		// P4-5：令牌缓存失效失败仅 SysError，不阻断权限更新流程（user.Update 已落库）。
		//   但若未及时刷新，被禁用/降权用户的令牌可能在 TTL 内仍持有旧权限发起请求，
		//   运维需通过本告警核对 Redis 状态并人工 FLUSHDB 兜底。
		if err := model.InvalidateUserTokensCache(user.Id); err != nil {
			common.SysError(fmt.Sprintf(
				"InvalidateTokensCache failed: user_id=%d action=%s role=%d cache_invalidate_required=true err=%v",
				user.Id, req.Action, user.Role, err,
