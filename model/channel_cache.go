...
// InitChannelCache 从数据库加载渠道与能力到内存缓存。
// P4-2 修复：返回 error 以区分"无需初始化（MemoryCache 关闭）"与"DB 查询失败"。
// - MemoryCache 未启用：返回 nil（非错误）
// - DB 查询失败：记录 SysError 并返回 error，调用方决定是否终止流程
func InitChannelCache() error {
	...
	if err := DB.Find(&channels).Error; err != nil {
		common.SysError(fmt.Sprintf("InitChannelCache: failed to load channels from db: %v", err))
		return err
	}
	...
	if err := DB.Find(&abilities).Error; err != nil {
		common.SysError(fmt.Sprintf("InitChannelCache: failed to load abilities from db: %v", err))
		return err
	}
...
// SyncChannelCache 定期同步渠道缓存。
// P1 修复：接受 context，服务退出时可通过 ctx.Done() 停止循环。
func SyncChannelCache(ctx context.Context, frequency int) {
	for {
		if !common.SleepWithContext(ctx, time.Duration(frequency)*time.Second) {
			common.SysLog("SyncChannelCache stopping: context cancelled")
			return
		}
		common.SysLog("syncing channels from database")
		if err := InitChannelCache(); err != nil {
			// P4-2 修复：DB 错误记录后继续下一轮循环，等待下次重试。
			common.SysError(fmt.Sprintf("SyncChannelCache: InitChannelCache failed: %v", err))
		}
	}
}
