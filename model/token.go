...
// DecreaseTokenQuotaSafe 原子扣减令牌额度（CAS 乐观锁）。
//
// 与 DecreaseTokenQuota 的关键区别：
//  1. 使用 `WHERE id = ? AND remain_quota >= ?` 条件更新，单条 SQL 原子完成"检查+扣减"，
//     消除原 PreConsumeTokenQuota 中 GetTokenByKey 检查 + DecreaseTokenQuota 之间的 TOCTOU 竞态。
//  2. 强制直接写 DB，绕过 BatchUpdate 队列。
//  3. DB 扣减成功后才异步扣减 Redis 缓存。
//
// 返回：
//   - nil：扣减成功
//   - ErrInsufficientTokenQuota：余额不足（RowsAffected == 0）
//   - 其他错误：数据库异常
//
// 注意：TokenUnlimited 的令牌应在调用前判断，本函数不做无限额度判断。
func DecreaseTokenQuotaSafe(id int, key string, quota int) (err error) {
	if quota < 0 {
		return errors.New("quota 不能为负数！")
	}
	if quota == 0 {
	...
	if common.RedisEnabled {
		gopool.Go(func() {
			if err := cacheDecrTokenQuota(key, int64(quota)); err != nil {
				common.SysLog(fmt.Sprintf("failed to decrease token quota cache (tokenId=%d, quota=%d): %s", id, quota, err.Error()))
			}
		})
	}
	return nil
