...
var (
	...
	// 订阅预扣费 / 结算相关哨兵错误
	// P4-3 修复：service 层使用 errors.Is 替代字符串匹配
	ErrNoActiveSubscription          = errors.New("no active subscription")          // 用户无 active 订阅
	ErrSubscriptionQuotaInsufficient = errors.New("subscription quota insufficient") // 所有订阅余额不足
	ErrSubscriptionUsedExceedsTotal  = errors.New("subscription used exceeds total") // PostConsume 超扣
)
...
// GetSubscriptionOrderByTradeNo 根据订单号查询订阅订单。
// P4-2 修复：返回 (*SubscriptionOrder, error) 以区分"记录不存在"与"DB 查询错误"。
...
// getSubscriptionPlanByIdTxNoCache 在数据库事务内通过 tx 读取套餐，不触碰 Redis 缓存。
//
// 为什么需要这个变体：
//  1. getSubscriptionPlanByIdTx 内部会调用 cachex.HybridCache（Redis GET/SET，每次 2s 超时），
//     若在 DB.Transaction 内调用，事务持锁期间会发起网络请求，最坏情况持锁可达数秒，
//     引发锁等待与连接池耗尽。
//  2. 本函数仅用事务自身的 tx 执行一次 First 查询，保证事务内读取的套餐与事务快照一致，
//     同时完全避免事务内的网络 IO。
//  3. 缓存会在后续非事务路径的读请求中自然回填，无需在此处写入。
func getSubscriptionPlanByIdTxNoCache(tx *gorm.DB, id int) (*SubscriptionPlan, error) {
	if id <= 0 {
		return nil, errors.New("invalid plan id")
	}
	var plan SubscriptionPlan
...
func CompleteSubscriptionOrder(tradeNo string, providerPayload string, expectedPaymentProvider string, actualPaymentMethod string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 事务内使用无缓存变体读取套餐，避免持锁期间发起 Redis 请求
		plan, err := getSubscriptionPlanByIdTxNoCache(tx, order.PlanId)
		if err != nil {
			return err
		}
		if !plan.Enabled {
...
func PurchaseSubscriptionWithBalance(userId int, planId int, tenantId int) error {
	...
	if chargedQuota > 0 {
		if err := cacheDecrUserQuota(userId, int64(chargedQuota)); err != nil {
			// P4-5：DB 已扣减 quota 但 Redis 缓存未同步，造成缓存与 DB 不一致。
			//   升级为 SysError + cache_sync_required=true 触发告警对账；
			//   不修改缓存策略，保持原有不阻断订阅主流程的行为。
			common.SysError(fmt.Sprintf(
				"cacheDecrUserQuota failed after subscription balance purchase: user_id=%d quota=%d cache_sync_required=true err=%v",
				userId, chargedQuota, err,
			))
		}
	}
	if upgradeGroup != "" {
	...
	for userId := range userIds {
		...
		err := DB.Transaction(func(tx *gorm.DB) error {
			...
			expiredQuery := tx.Where("user_id = ? AND status = ? AND upgrade_group <> ''",
				...
				Limit(1).
				Find(&lastExpired)
			// P4-5 修复：DB error 必须回滚事务，禁止用 nil 静默吞掉。
			// 原逻辑 `expiredQuery.Error != nil || RowsAffected == 0` 把 DB error 与"无记录"混为一谈，
			// 导致事务正常提交，用户组降级被永久跳过（status 已变 expired，下次扫描不再处理该 userId）。
...
func PreConsumeUserSubscription(requestId string, userId int, modelName string, quotaType int, amount int64, tenantId int) (*SubscriptionPreConsumeResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, candidate := range subs {
			sub := candidate
			// 事务内（且在 FOR UPDATE 持锁循环中）使用无缓存变体读取套餐，
			// 避免每轮循环触发 Redis 请求导致持锁时间不可控
			plan, err := getSubscriptionPlanByIdTxNoCache(tx, sub.PlanId)
			if err != nil {
				return err
			}
...
func RefundSubscriptionPreConsume(requestId string) error {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		// P3 并发修复：原调用 PostConsumeUserSubscriptionDelta 会开启独立事务，
		//   外层事务已持有 record 锁，内层新事务再竞争 subscription 的 FOR UPDATE 锁，
		//   与 PreConsumeUserSubscription 的锁顺序（subscription → record）相反，
		//   高并发下可能死锁。现复用外层 tx，锁顺序统一为 record → subscription，
		//   全流程同一事务，保留 FOR UPDATE / 幂等 / rollback 行为。
...
// postConsumeUserSubscriptionDeltaWithTx 是 PostConsumeUserSubscriptionDelta 的事务内联版本。
// 调用方必须已在事务中并持有所需锁；本函数只对 subscription 加 FOR UPDATE 并更新 AmountUsed。
// 抽取目的：RefundSubscriptionPreConsume 复用外层事务，避免嵌套事务死锁。
func postConsumeUserSubscriptionDeltaWithTx(tx *gorm.DB, userSubscriptionId int, delta int64) error {
	...
	if sub.AmountTotal > 0 && newUsed > sub.AmountTotal {
		return fmt.Errorf("%w: used=%d total=%d", ErrSubscriptionUsedExceedsTotal, newUsed, sub.AmountTotal)
	}
