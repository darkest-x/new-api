...
// MaxChannelSearchResults 限制管理后台渠道搜索/按 tag 查询的最大返回记录数，
// 防止宽泛关键词或大 tag 下全量加载导致慢查询与内存膨胀。
const MaxChannelSearchResults = 1000
	...
	case constant.MultiKeyModePolling:
		...
		defer func() {
			...
			if !common.MemoryCacheEnabled {
				// P4-5 修复：原 `_ = SaveChannelInfo()` 静默丢弃 polling index 持久化失败，
				// 进程重启或缓存刷新后从 DB 读取到旧位置 → 多 key 间负载严重不均。
				// 此处位于 defer，无法向上返回 error；保留 SaveChannelInfo 调用并升级日志为 SysError，
				// 让运维可定位 channel_id 与失败原因，必要时手动重启或重置 MultiKeyPollingIndex。
				if saveErr := channel.SaveChannelInfo(); saveErr != nil {
					common.SysError(fmt.Sprintf("GetNextEnabledKeyForChannel: SaveChannelInfo failed: channel_id=%d, polling_index=%d, err=%v",
						channel.Id, channel.ChannelInfo.MultiKeyPollingIndex, saveErr))
				}
			} else {
...
func GetChannelsByTag(tag string, idSort bool, selectAll bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	// 强制加上限，防止某 tag 下渠道数量过大导致全量加载
	query = query.Limit(MaxChannelSearchResults)
...
func SearchChannels(keyword string, group string, model string, idSort bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	// 执行查询，强制加上限防止宽泛关键词命中过多记录
	err := order.Apply(baseQuery).Limit(MaxChannelSearchResults).Find(&channels).Error
	if err != nil {
		return nil, err
	}
	return channels, nil
...
func BatchDeleteChannels(ids []int) (err error) {
	if len(ids) == 0 {
		return nil
	}
	...
	// P3-5 修复：增加 defer recover，panic 时确保事务 rollback，避免连接泄漏。
	defer func() {
		if r := recover(); r != nil {
			tx.Rollback()
			panic(r) // 重新抛出，让上层 gin Recovery 中间件处理
		}
	}()
...
func (channel *Channel) Update() error {
	...
	// P3-4 修复：Updates 后需重新加载最新数据供 UpdateAbilities 使用，
	// 此前直接丢弃 First 错误，DB 异常时会让 channel 处于部分更新的脏数据状态，
	// 进而导致 UpdateAbilities 写入错误的 ability 数据。
	if err := DB.Model(channel).First(channel, "id = ?", channel.Id).Error; err != nil {
		// 刚刚 Updates 成功，理论上记录必然存在；这里 ErrRecordNotFound 与真实 DB 错误
		// 都属于异常路径，必须记录并返回，避免后续基于空/陈旧数据继续操作。
		common.SysError(fmt.Sprintf("Channel.Update reload failed: channel_id=%d, error=%s",
			channel.Id, err.Error()))
		return fmt.Errorf("reload channel after update failed: %w", err)
	}
...
func CleanupChannelPollingLocks() {
	...
	// P4-4 修复：禁止丢弃 Pluck 错误。
	// DB 查询失败时 activeChannelIds 为空会导致所有 polling locks 被误删，
	// 破坏多 key 模式轮询的互斥保护，必须记录并直接返回。
	if err := DB.Model(&Channel{}).Pluck("id", &activeChannelIds).Error; err != nil {
		common.SysError(fmt.Sprintf("CleanupChannelPollingLocks: Pluck channel ids failed: %v", err))
		return
	}
...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	if common.MemoryCacheEnabled {
		...
		// P4-4 修复：禁止丢弃 CacheGetChannel 错误。
		// DB 查询失败时 channelCache 为 nil 会让本应成功的状态更新被静默跳过，
		// 必须记录并返回 false 让调用方知道更新未生效。
		channelCache, err := CacheGetChannel(channelId)
		if err != nil {
			common.SysError(fmt.Sprintf("UpdateChannelStatus: CacheGetChannel failed: channel_id=%d, error=%v", channelId, err))
			return false
		}
		if channelCache == nil {
			return false
	...
	if err != nil {
		// P4-2 修复：区分"渠道不存在"与"DB 查询错误"，后者必须记录避免静默失败。
		if !errors.Is(err, gorm.ErrRecordNotFound) {
			common.SysError(fmt.Sprintf("UpdateChannelStatus: GetChannelById db error: channel_id=%d, error=%v", channelId, err))
		}
		return false
	} else {
...
func EditChannelByTag(tag string, newTag *string, modelMapping *string, models *string, group *string, priority *int64, weight *uint, paramOverride *string, headerOverride *string) error {
	...
	if shouldReCreateAbilities {
		// P4-5 修复：禁止丢弃 GetChannelsByTag 的 DB error。
		// 原逻辑 `if err == nil` 在 DB 异常时静默跳过 abilities 重建，
		// 会让 abilities 表与 channel 配置不一致，影响选道。
		// 注意：GORM Find 在无记录时返回空切片且 err==nil，
		// 此处的 err 一定是真实 DB error（非 ErrRecordNotFound）。
		channels, err := GetChannelsByTag(updatedTag, false, false)
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				// 理论上 Find 不会返回 ErrRecordNotFound，但保守处理。
				common.SysError(fmt.Sprintf("EditChannelByTag: GetChannelsByTag no records: tag=%s, err=%v", updatedTag, err))
			} else {
				return fmt.Errorf("EditChannelByTag: GetChannelsByTag failed: tag=%s, err=%w", updatedTag, err)
			}
		} else {
			// UpdateAbilities 单个失败不能静默跳过：记录 SysError 并聚合错误返回。
			var abilitiesErrs []error
			for _, channel := range channels {
				if updateErr := channel.UpdateAbilities(nil); updateErr != nil {
					common.SysError(fmt.Sprintf("EditChannelByTag: failed to update abilities: channel_id=%d, tag=%s, error=%v", channel.Id, channel.GetTag(), updateErr))
					abilitiesErrs = append(abilitiesErrs, fmt.Errorf("channel_id=%d: %w", channel.Id, updateErr))
				}
			}
			if len(abilitiesErrs) > 0 {
				return fmt.Errorf("EditChannelByTag: %d channel(s) failed to update abilities: %w", len(abilitiesErrs), errors.Join(abilitiesErrs...))
			}
		}
	} else {
...
func (channel *Channel) GetSetting() dto.ChannelSettings {
	...
	if channel.Setting != nil && *channel.Setting != "" {
		...
		if err != nil {
			...
			// P4-5 修复：原 `_ = Save()` 静默丢弃持久化失败，DB 中仍是损坏 JSON，
			// 下次读取再次失败 → 死循环。函数签名为 dto.ChannelSettings（无 error 返回），
			// 且有 15+ 调用方（middleware/controller/relay/service 多处），
			// 修改签名会扩大影响范围。保留签名，但升级日志为 SysError 并附带完整上下文，
			// 让运维可定位 channel_id 并手动修复损坏的 Setting 字段。
			if saveErr := channel.Save(); saveErr != nil {
				common.SysError(fmt.Sprintf("GetSetting: failed to persist cleared Setting (corrupted JSON remains in DB): channel_id=%d, save_err=%v, unmarshal_err=%v",
					channel.Id, saveErr, err))
			}
		}
	}
	return setting
...
func (channel *Channel) GetOtherSettings() dto.ChannelOtherSettings {
	...
	if channel.OtherSettings != "" {
		...
		if err != nil {
			...
			// P4-5 修复：同 GetSetting，原 `_ = Save()` 静默丢弃持久化失败。
			// 函数签名无 error 返回，且有 10+ 调用方（controller/channel.go、
			// controller/channel_upstream_update.go、middleware/distributor.go 等）。
			// 保留签名，升级日志为 SysError 让运维可定位并手动修复损坏的 OtherSettings 字段。
			if saveErr := channel.Save(); saveErr != nil {
				common.SysError(fmt.Sprintf("GetOtherSettings: failed to persist cleared OtherSettings (corrupted JSON remains in DB): channel_id=%d, save_err=%v, unmarshal_err=%v",
					channel.Id, saveErr, err))
			}
		}
	}
...
func BatchSetChannelTag(ids []int, tag *string) (err error) {
	// 开启事务
	tx := DB.Begin()
	if tx.Error != nil {
		return tx.Error
