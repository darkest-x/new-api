...
func TestRechargeWaffoPancake_RejectsMismatchedPaymentMethod(t *testing.T) {
	...
	topUp, err := GetTopUpByTradeNo("waffo-pancake-guard")
	require.NoError(t, err)
	require.NotNil(t, topUp)
...
func TestCompleteSubscriptionOrder_RejectsMismatchedPaymentProvider(t *testing.T) {
	...
	order, err := GetSubscriptionOrderByTradeNo("sub-guard-order")
	require.NoError(t, err)
	require.NotNil(t, order)
	assert.Equal(t, common.TopUpStatusPending, order.Status)
	assert.Zero(t, countUserSubscriptionsForPaymentGuardTest(t, 202))

	topUp, err := GetTopUpByTradeNo("sub-guard-order")
	require.NoError(t, err)
	assert.Nil(t, topUp)
}
...
func TestExpireSubscriptionOrder_RejectsMismatchedPaymentProvider(t *testing.T) {
	...
	order, err := GetSubscriptionOrderByTradeNo("sub-expire-guard")
	require.NoError(t, err)
	require.NotNil(t, order)
	assert.Equal(t, common.TopUpStatusPending, order.Status)
}
