...
func RelayMidjourneyImage(c *gin.Context) {
	taskId := c.Param("id")
	// P4-3 修复：接收 error 以区分"任务不存在"与"DB 查询错误"
	midjourneyTask, err := model.GetByOnlyMJId(taskId)
	if err != nil {
		// DB error 不当作任务不存在处理，避免静默吞掉错误
		logger.LogError(c, fmt.Sprintf("RelayMidjourneyImage: GetByOnlyMJId DB error taskId=%s err=%v", taskId, err))
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "get_midjourney_task_failed",
		})
		return
	}
	...
	if err != nil {
		logger.LogError(nil, "failed to stream image: "+err.Error())
	}
	return
}
...
func RelayMidjourneyNotify(c *gin.Context) *dto.MidjourneyResponse {
	...
	midjourneyTask, err := model.GetByOnlyMJId(midjRequest.MjId)
	if err != nil {
		// P4-3 修复：DB error 不当作任务不存在处理，避免静默吞掉错误
		logger.LogError(c, fmt.Sprintf("RelayMidjourneyNotify: GetByOnlyMJId DB error mjId=%s err=%v", midjRequest.MjId, err))
		return &dto.MidjourneyResponse{
			Code:        4,
			Description: "get_midjourney_task_failed",
			Properties:  nil,
			Result:      "",
		}
	}
...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			if err != nil {
				// P4-5：消费扣费失败但下方 RecordConsumeLog 仍会写入 Quota=priceData.Quota 的消费记录，
				//   造成"日志显示已消费但用户额度未扣减"的审计断裂。
				//   升级为 SysError + billing_log_inconsistent=true 触发告警对账；
				//   不新增 return，不改变 relay 返回逻辑。
				common.SysError(fmt.Sprintf(
					"MjProxy SwapFace: consume quota failed: consume_required=true billing_log_inconsistent=true user_id=%d token_id=%d channel_id=%d model_name=%s quota=%d err=%v",
					info.UserId, info.TokenId, info.ChannelId, modelName, priceData.Quota, err,
				))
			}
			...
			if err := model.RecordConsumeLog(c, info.UserId, model.RecordConsumeLogParams{
				...
				// P4-5: MJ swapface 消费日志写入失败不阻塞流程，但必须 SysError 记录上下文供对账。
				common.SysError(fmt.Sprintf(
					"MjProxy swapface: consume log persist failed: log_persist_required=true, user_id=%d, token_id=%d, channel_id=%d, model_name=%s, quota=%d, log_type=%d, err=%v",
					info.UserId, info.TokenId, info.ChannelId, modelName, priceData.Quota, model.LogTypeConsume, err,
				))
			}
...
func RelayMidjourneyTaskImageSeed(c *gin.Context) *dto.MidjourneyResponse {
	...
	userId := c.GetInt("id")
	// P4-3 修复：接收 error 以区分"任务不存在"与"DB 查询错误"
	originTask, err := model.GetByMJId(userId, taskId)
	if err != nil {
		// DB error 不当作任务不存在处理，避免静默吞掉错误
		logger.LogError(c, fmt.Sprintf("RelayMidjourneyTaskImageSeed: GetByMJId DB error userId=%d taskId=%s err=%v", userId, taskId, err))
		return service.MidjourneyErrorWrapper(constant.MjRequestError, "get_midjourney_task_failed")
	}
	if originTask == nil {
...
func RelayMidjourneyTask(c *gin.Context, relayMode int) *dto.MidjourneyResponse {
	...
	case relayconstant.RelayModeMidjourneyTaskFetch:
		...
		if err != nil {
			// DB error 不当作任务不存在处理，避免静默吞掉错误
			logger.LogError(c, fmt.Sprintf("RelayMidjourneyTask Fetch: GetByMJId DB error userId=%d taskId=%s err=%v", userId, taskId, err))
			return &dto.MidjourneyResponse{
				Code:        4,
				Description: "get_midjourney_task_failed",
			}
	...
	case relayconstant.RelayModeMidjourneyTaskFetchByCondition:
		...
		if len(condition.IDs) != 0 {
			...
			originTasks, err := model.GetByMJIds(userId, condition.IDs)
			if err != nil {
				// DB error 不当作无任务处理，避免静默吞掉错误
				logger.LogError(c, fmt.Sprintf("RelayMidjourneyTask FetchByCondition: GetByMJIds DB error userId=%d err=%v", userId, err))
				return &dto.MidjourneyResponse{
					Code:        4,
					Description: "get_midjourney_task_failed",
				}
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if relayInfo.RelayMode == relayconstant.RelayModeMidjourneyImagine { //绘画任务，此类任务可重复
		...
		// P4-3 修复：接收 error 以区分"任务不存在"与"DB 查询错误"
		originTask, err := model.GetByMJId(relayInfo.UserId, mjId)
		if err != nil {
			// DB error 不当作任务不存在处理，避免静默吞掉错误
			logger.LogError(c, fmt.Sprintf("RelayMidjourneySubmit: GetByMJId DB error userId=%d mjId=%s err=%v", relayInfo.UserId, mjId, err))
			return service.MidjourneyErrorWrapper(constant.MjRequestError, "get_midjourney_task_failed")
		}
		if originTask == nil {
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			if err != nil {
				...
				//   不新增 return，不改变 relay 返回逻辑。
				common.SysError(fmt.Sprintf(
					"MjProxy action: consume quota failed: consume_required=true billing_log_inconsistent=true user_id=%d token_id=%d channel_id=%d model_name=%s quota=%d err=%v",
					relayInfo.UserId, relayInfo.TokenId, relayInfo.ChannelId, modelName, priceData.Quota, err,
				))
			}
			tokenName := c.GetString("token_name")
			...
			if err := model.RecordConsumeLog(c, relayInfo.UserId, model.RecordConsumeLogParams{
				...
				// P4-5: MJ action 消费日志写入失败不阻塞流程，但必须 SysError 记录上下文供对账。
				common.SysError(fmt.Sprintf(
					"MjProxy action: consume log persist failed: log_persist_required=true, user_id=%d, token_id=%d, channel_id=%d, model_name=%s, quota=%d, log_type=%d, err=%v",
					relayInfo.UserId, relayInfo.TokenId, relayInfo.ChannelId, modelName, priceData.Quota, model.LogTypeConsume, err,
				))
			}
