					...
					if err == nil && preferred != nil && preferred.Status == common.ChannelStatusEnabled {
						if usingGroup == "auto" {
							...
							for _, g := range autoGroups {
								// P4-4 修复：IsChannelEnabledForGroupModel 返回 (bool, error)。
								// DB error 不阻断主流程，记录后继续 fallback 到下一个 group 或随机选道。
								enabled, dbErr := model.IsChannelEnabledForGroupModel(g, modelRequest.Model, preferred.Id)
								if dbErr != nil {
									common.SysError(fmt.Sprintf("distributor: IsChannelEnabledForGroupModel DB error: group=%s, model=%s, channel_id=%d, err=%v",
										g, modelRequest.Model, preferred.Id, dbErr))
								} else if enabled {
									selectGroup = g
							...
							// P4-4 修复：IsChannelEnabledForGroupModel 返回 (bool, error)。
							// DB error 不阻断主流程，记录后继续 fallback 到随机选道。
							enabled, dbErr := model.IsChannelEnabledForGroupModel(usingGroup, modelRequest.Model, preferred.Id)
							if dbErr != nil {
								common.SysError(fmt.Sprintf("distributor: IsChannelEnabledForGroupModel DB error: group=%s, model=%s, channel_id=%d, err=%v",
									usingGroup, modelRequest.Model, preferred.Id, dbErr))
							} else if enabled {
								channel = preferred
